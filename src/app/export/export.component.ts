...
export class ExportComponent implements OnInit {
    ...
    public statusTypeOptions: Array<any> = [{}];
    ...
    public errorMessage = {
        ...
        statusTypeRequired: 'Status is required',
    };
    ...
    ngOnInit() {
        ...
        this.reservationService.getDataForFilter().subscribe(
            res => {
                ...
                this.statusTypeOptions = this.formatInputSelect2(res.json().reservationStatuses, null);
                ...
                this.filterObj.statusType = this.statusTypeOptions[0].id;
        ...
        if (this.validFilterModel().length == 0) {
            ...
            this.exportService.downloadFile(this.convertToBE(this.filterObj)).delay(300).subscribe(
                res => {
                    ...
                    this.blobFile = res;
                    let filename;
                    filename = this.filterObj.statusType == this.statusTypeOptions[1].id ? 'crm_empty_dormitory_data.csv':'crm_dormitory_data.csv';
                    this.pDownloadFile(res, (this.filterObj.stayType == this.stayTypeOptions[0].id) ? filename : "crm_others_data.csv");
                },
                error => {
                    this.isModalShown = false;
                    if (error.status == 500) {
    ...
    private validFilterModel() {
        ...
        if (!this.filterObj.statusType) {
            error.push(this.errorMessage.statusTypeRequired);
        }
    ...
    private convertToBE(frontEndModel: ExportFilterModel): ExportFilterModel {
        ...
        backendModel.statusType = frontEndModel.statusType;
