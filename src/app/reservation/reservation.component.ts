...
export class ReservationComponent implements OnInit {
  ...
  public stayPlanDefined = [{ id: 1, name: 'Dormitory' }, { id: 2, name: 'Others' },
      { id: 3, name: 'Condo' }, { id: 4, name: 'Hotel' }, { id: 5, name: 'Walk-in' }];
  public AppSettings;
  ...
  public checkDateDuplicateOrNot(id, date, bedId, type) {
    ...
    this.reservationService.checkDateDuplicateOrNot(obj,this.filterObj).subscribe(
      res => {
        if (res.json().result) {
          if (obj.type === 1) {
  ...
  public initDomitory(obj, dateFrom, dateTo, reservationInfo) {
    if (obj && obj.reservationDate && obj.reservationDate.id) {
      ...
      this.reservation = new ReservationDateModel();
      this.reservationService.getReservation(obj.reservationDate.id, this.filterObj).subscribe(
        res => {
  ...
  public updateReservation(reservation) {
    ...
    setTimeout(() => {
      ...
      if (error) {
        ...
        this.reservationService.updateReservation(reservation.id, reservation, this.filterObj).subscribe(
          res => {
  ...
  public createReservation(reservation) {
    ...
    setTimeout(() => {
      ...
      if (error) {
        ...
        this.reservationService.createReservation(reservation, this.filterObj).subscribe(
          res => {
            this.reservation = res.json() as ReservationDateModel;
            this.calendar(this.startDate, this.filterObj);
            this.warningModal.hide();
  ...
  public deleteReservation(id) {
    ...
    setTimeout(() => {
      ...
      if (!this.isOtherReservation) {
        this.reservationService.deleteReservation(id, objLastUpdated, this.filterObj).subscribe(
          res => {
            this.reservation = new ReservationDateModel;
            this.calendar(this.startDate, this.filterObj);
