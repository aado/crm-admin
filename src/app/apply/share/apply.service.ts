...
export class ApplyService {
    ...
    //// convert currency check if under maintenance
    checkCurrencyUnderMaintenance(): Observable<Response> {
        const url = 'app/admin/apply/check-currency-if-maintenance';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url, this.getHeader())
    ...
    uploadInvoiceData(file: File, applyId): Observable<Response> {
        const url_invoice_upload = 'app/admin/apply/invoice-file-upload';
        const formData: FormData = new FormData();
        formData.append('invoicefile', file);
        if (this.configService.config) {
            this.AppSettings = this.configService.config;
            return this.http.post(this.AppSettings['API_ENDPOINT'] + url_invoice_upload + '/' + applyId, formData, this.options).map((res: Response) => res.json());
        }
        return this.configService.loadConfiguration().mergeMap(() => {
            this.AppSettings = this.configService.config;
            return this.http.post(this.AppSettings['API_ENDPOINT'] + url_invoice_upload, formData, this.options).map((res: Response) => res.json());
        });
    }

    downloadInvoiceData(filterObj: ApplySearchModel, applyId): Observable<Response> {
        const url_invoice_download = 'app/admin/apply/invoice-file-download';
        if (this.configService.config) {
            return this.createObservable(filterObj, this.configService.config, url_invoice_download + '/' + applyId);
        } else {
            return this.configService.loadConfiguration().mergeMap(() => {
                return this.createObservable(filterObj, this.configService.config, url_invoice_download + '/' + applyId);
            });
    ...
    uploadExtensionInvoiceData(file: File, invoiceId): Observable<Response> {
        const url_invoice_upload = 'app/admin/apply/ext-invoice-file-upload';
        const formData: FormData = new FormData();
        formData.append('extinvoicefile', file);
        if (this.configService.config) {
            this.AppSettings = this.configService.config;
            return this.http.post(this.AppSettings['API_ENDPOINT'] + url_invoice_upload + '/' + invoiceId, formData, this.options).map((res: Response) => res.json());
        }
        return this.configService.loadConfiguration().mergeMap(() => {
            this.AppSettings = this.configService.config;
            return this.http.post(this.AppSettings['API_ENDPOINT'] + url_invoice_upload, formData, this.options).map((res: Response) => res.json());
    ...
    downloadExtensionInvoiceData(filterObj: ApplySearchModel, invoiceId): Observable<Response> {
        const url_invoice_download = 'app/admin/apply/ext-invoice-file-download';
        if (this.configService.config) {
            return this.createObservable(filterObj, this.configService.config, url_invoice_download + '/' + invoiceId);
        } else {
            return this.configService.loadConfiguration().mergeMap(() => {
                return this.createObservable(filterObj, this.configService.config, url_invoice_download + '/' + invoiceId);
            });
    ...
    uploadRefundInvoiceData(file: File, invoiceId): Observable<Response> {
        const url_invoice_upload = 'app/admin/apply/rfd-invoice-file-upload';
        const formData: FormData = new FormData();
        formData.append('rfdinvoicefile', file);
        if (this.configService.config) {
            this.AppSettings = this.configService.config;
            return this.http.post(this.AppSettings['API_ENDPOINT'] + url_invoice_upload + '/' + invoiceId, formData, this.options).map((res: Response) => res.json());
        }
    ...
    downloadRefundInvoiceData(filterObj: ApplySearchModel, invoiceId): Observable<Response> {
        const url_invoice_download = 'app/admin/apply/rfd-invoice-file-download';
        if (this.configService.config) {
            return this.createObservable(filterObj, this.configService.config, url_invoice_download + '/' + invoiceId);
        } else {
            return this.configService.loadConfiguration().mergeMap(() => {
    ...
    getInvoicesList(applyId): Observable<Response> {
        const url_find_list_invoices = 'app/admin/apply/find-list-invoices';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_find_list_invoices + '/' + applyId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }

    getExtInvoicesList(applyId): Observable<Response> {
        const url_find_list_extension_invoices = 'app/admin/apply/find-list-extension-invoices';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_find_list_extension_invoices + '/' + applyId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }

    getRfdInvoicesList(applyId): Observable<Response> {
        const url_find_list_refund_invoices = 'app/admin/apply/find-list-refund-invoices';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_find_list_refund_invoices + '/' + applyId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    getExtenionInvoice(applyId, invoiceId): Observable<Response> {
        const url_extension_invoice = 'app/admin/apply/find-extension-invoice';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_extension_invoice + '/' + applyId + '/' + invoiceId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    getExtenionInvoicePhpRate(applyId,invoiceId): Observable<Response> {
        const url_extension_invoice_php_rate = 'app/admin/apply/find-extension-invoice-php-rate';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_extension_invoice_php_rate + '/' + applyId + '/' + invoiceId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    updateExtensionInvoiceDiscount(applyId, invoiceId, totalInvoice): Observable<Response> {
        const url_update_ext_inv_discount = 'app/admin/apply/update-ext-invoice-discount';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_update_ext_inv_discount + '/' + applyId + '/' + invoiceId + '/' + totalInvoice, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    updateApplyExtensionInvCommission(applyId, invoiceId, commission, commission_percentage): Observable<Response> {
        const url_update_ext_inv_commission = 'app/admin/apply/update-apply-ext-inv-commission';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_update_ext_inv_commission + '/' + applyId + '/' + invoiceId + '/' + commission + '/' + commission_percentage, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    updateApplyRefundInvDiscount(applyId, refund, refund_percentage, condition, reason): Observable<Response> {
        const url_update_rfd_inv_refund_discount = 'app/admin/apply/update-apply-rfd-inv-refund-discout';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_update_rfd_inv_refund_discount + '/' + applyId + '/' + refund + '/' + refund_percentage + '/' + condition + '/' + reason, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    deleteExtendedInvoiceNumber(applyId, invoiceId): Observable<Response> {
        const url_remove_ext_invoice_number = 'app/admin/apply/delete-extended-invoice-number';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_remove_ext_invoice_number + '/' + applyId + '/' + invoiceId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    updateIsExtInvoiceConfirmedFlag(ApplyId, InvoiceId, invoiceExtLockStatus): Observable<Response> {
        const url_ext_invoice_lock_update = 'app/admin/apply/is-ext-invoice-confirmed-update';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_ext_invoice_lock_update + '/' + ApplyId + "/" + InvoiceId + "/"  + invoiceExtLockStatus, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
    ...
    addExtensionInvoice(applyId): Observable<Response> {
        const url_create_extension_invoice = 'app/admin/apply/create-extension-invoice';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_create_extension_invoice + '/' + applyId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }

    addRefundInvoice(applyId): Observable<Response> {
        const url_create_refund_invoice = 'app/admin/apply/create-refund-invoice';
        return this.checkConfigJson(() => {
            return this.http.get(this.AppSettings['API_ENDPOINT'] + url_create_refund_invoice + '/' + applyId, this.getHeader())
                .map((res: Response) => res)
                .catch((error: any) => Observable.throw(error.json().errors || 'Server error'));
        });
    }
